...
const handleRequest = async (listDetails, callback) => {
  ...
  if (!user) {
    ...
    callback({ message: 'You cannot subscribe to your own list!' }, null);
  } else if (_.find(user.subscribedLists, new ObjectId(listDetails.listId))) {
    callback({ message: 'You have already subscribed to the list!' }, null);
  } else {
    user.subscribedLists.addToSet(listDetails.listId);
    user.save();
    callback(null, { message: 'Subscribed to List' });
  }
};
